...
class ServoHold90Node(Node):
    def __init__(self):
        ...
        self.angles = self.parse_angles(self.get_parameter("angles").value)
    ...
    def parse_angles(self, value):
        if isinstance(value, str):
            text = value.strip()
            if text.startswith("[") and text.endswith("]"):
                text = text[1:-1]
            angles = [int(part.strip()) for part in text.split(",") if part.strip()]
        else:
            angles = [int(angle) for angle in value]

        if len(angles) != 5:
            raise ValueError(f"angles must contain 5 values, got {angles}")

        invalid = [angle for angle in angles if angle < 0 or angle > 180]
        if invalid:
            raise ValueError(f"angles must be in 0..180, got {angles}")
