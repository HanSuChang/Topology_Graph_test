...
public:
  ...
  : Node("rc_car_follower_node")
  {
    ...
    this->declare_parameter<double>("slot_staging_tolerance", 0.08);
    this->declare_parameter<double>("slot_yaw_tolerance", 0.16);
    this->declare_parameter<double>("slot_drive_heading_limit", 0.35);
    this->declare_parameter<double>("slot_realign_heading_limit", 1.20);
    ...
    this->declare_parameter<bool>("enable_rc_a_stop_follow_hold", true);
    this->declare_parameter<double>("rc_a_stop_follow_tolerance", 0.10);
    ...
    slot_realign_heading_limit_ = std::max(
      slot_drive_heading_limit_, this->get_parameter("slot_realign_heading_limit").as_double());
    ...
    enable_rc_a_stop_follow_hold_ =
      this->get_parameter("enable_rc_a_stop_follow_hold").as_bool();
    rc_a_stop_follow_tolerance_ = std::max(
      0.02, this->get_parameter("rc_a_stop_follow_tolerance").as_double());
...
private:
  ...
  void set_mode(const std::string & mode)
  {
    ...
    if (mode == "follow") {
      ...
      clear_intersection_1_left_turn();
    } else if (mode == "slot_wait_a" || mode == "slot_wait_b") {
      mode_ = FollowerMode::Follow;
      ...
      RCLCPP_WARN(
        this->get_logger(),
        "Mode %s is disabled; using follow mode instead",
        mode.c_str());
    } else {
  ...
  void control_step()
  {
    ...
    if (mode_ == FollowerMode::Follow && enable_rc_a_stop_follow_hold_) {
      const double rc_a_stop_distance =
        std::hypot(follower.x - rc_a_stop_x_, follower.y - rc_a_stop_y_);
      if (rc_a_stop_distance <= rc_a_stop_follow_tolerance_ && !is_leader_moving()) {
        RCLCPP_INFO_THROTTLE(
          this->get_logger(),
          *this->get_clock(),
          1000,
          "Holding RC car at rc_car_a_stop without yaw alignment: distance=%.3f tolerance=%.3f",
          rc_a_stop_distance,
          rc_a_stop_follow_tolerance_);
        send_pwm_command(PwmCommand{0, 0});
        return;
      }
    }
  ...
  PwmCommand make_slot_wait_command(const Pose2D & follower)
  {
    ...
    if (slot_wait_stage_ == SlotWaitStage::DriveToIntersection) {
      if (distance <= slot_intersection_tolerance_) {
        ...
        publish_slot_wait_status(slot_wait_task_key() + "_aligning");
        RCLCPP_INFO(
          this->get_logger(),
    ...
    if (slot_wait_stage_ == SlotWaitStage::AlignToStaging) {
      if (std::abs(heading_error) <= slot_yaw_tolerance_) {
        ...
        publish_slot_wait_status(slot_wait_task_key() + "_driving");
    ...
    if (slot_wait_stage_ == SlotWaitStage::DriveToStaging) {
      ...
      if (std::abs(heading_error) > slot_realign_heading_limit_) {
        publish_slot_wait_status(slot_wait_task_key() + "_realigning");
        return make_slot_turn_command(heading_error);
      }
      return apply_turn_to_pwm(slot_forward_pwm_, heading_error, near_turn_pwm_delta_);
    }
  ...
  Pose2D slot_wait_staging_target() const
  {
    if (slot_wait_task_ == SlotWaitTask::A) {
      Pose2D target;
      target.x = rc_a_stop_x_;
      target.y = rc_a_stop_y_;
      target.yaw = 0.0;
      return target;
    }
  ...
  double slot_realign_heading_limit_{1.20};
  int slot_forward_pwm_{80};
  int slot_turn_pwm_{95};
  double rc_a_stop_x_{0.7064711451530457};
  double rc_a_stop_y_{0.29016929864883423};
  bool enable_rc_a_stop_follow_hold_{true};
  double rc_a_stop_follow_tolerance_{0.10};
